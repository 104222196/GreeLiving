...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (count($errors) === 0) {
        $statement = new mysqli_stmt($db, "UPDATE Applicant 
                                           ...
                                           WHERE ApplicantID = ?");
        $statement->bind_param("sssssssssssssss", $fName, $lName, $birthdate, $gender, $phone, $nationality, $countryOfResidence, $city, $district, $streetAddress,
                                $jobTitle, $experience, $education, $careerGoal, $_SESSION["applicantId"]);
        $success = $statement->execute();
        ...
        <select name="experience">
            <?php foreach ($validExperiences as $validExperience): ?>
                <option
                    ...
                    <?=($validExperience === $experience) ? " selected" : ""?>
                >
                    <?=$validExperience?>
                </option>
            <?php endforeach; ?>
        ...
        <select name="education">
            <?php foreach ($validEducationLevels as $validEducationLevel): ?>
                <option
                    ...
                    <?=($validEducationLevel === $education) ? " selected" : ""?>
                >
                    <?=$validEducationLevel?>
                </option>
            <?php endforeach; ?>
